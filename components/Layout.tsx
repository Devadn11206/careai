...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-gradient)] flex flex-col md:flex-row font-sans text-[var(--text-main)] relative overflow-hidden transition-colors duration-500">
      <HoloBackdrop3D className="opacity-40" intensity={0.4} />
      <div className="absolute inset-0 pointer-events-none aurora-bg opacity-30" />

      {/* Sidebar */}
      <nav className={`
        fixed md:relative z-30 flex flex-col h-screen transition-all duration-500 glass-card border-r border-[var(--glass-border)]
      ...
      `}>
        ...
        <div className="p-6">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[var(--accent-primary)] to-[var(--accent-secondary)] flex items-center justify-center shadow-lg animate-pulse">
              <span className="text-white font-black text-xl tracking-tighter italic">X</span>
            </div>
            <div>
              <h2 className="text-xl font-black font-orbitron tracking-tighter text-[var(--text-main)]">
                CARE<span className="premium-gradient-text">XAI</span>
              </h2>
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                <span className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)]">Core Systems Active</span>
              </div>
            </div>
          </div>
